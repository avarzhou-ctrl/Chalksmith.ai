...
`LLM_MAX_OUTPUT_TOKENS` caps generated output. Binary PDF and image inputs are instead bounded by `MAX_SOURCE_FILES`, `MAX_SOURCE_BYTES`, and `MAX_TOTAL_SOURCE_BYTES`; their billed input size depends on the provider's document and image tokenization. Lower the byte limits only after measuring real source-backed lessons, because overly small limits prevent otherwise valid classroom material from reaching the model.

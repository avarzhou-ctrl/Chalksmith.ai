...
| :--- | :--- | :--- |
| Slides | Declarative slide kinds and blocks | Reveal.js HTML with compiler-selected block layouts |
...
- Do not let the model emit global CSS, arbitrary HTML, Reveal configuration, or Manim positioning
  code in the new path.
...
- DOM structure and component markup;
...
- All user-visible strings are plain text or explicitly typed mathematics; raw HTML is rejected.
...
Slides have a finite visual grammar: title, explanation, comparison, diagram, worked example,
question, and recap. Asking the model to write arbitrary Reveal HTML and CSS creates variation
without adding teaching capability. The model should therefore return slide content and semantic
visuals; the platform should construct every `<section>` and initialize Reveal.
...
2. renders typed blocks into platform-owned markup;
...
- no slide contains unknown blocks or raw markup;
- compiled slides do not create scroll containers.

Because markup and CSS are deterministic, block-composition screenshots belong in CI. Per-generation
browser validation is a safety net for unusual text and mathematics, not a mechanism for redesigning
slides.
...
- Slides contain no model-authored HTML, JavaScript, SVG, or Reveal configuration;
...
- **No arbitrary escape hatch in v1:** missing capability becomes a new typed primitive or
  format-owned composition rule.

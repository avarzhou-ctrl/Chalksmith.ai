"""Allowlist sanitizer for model-authored slide HTML.

Generated markup is untrusted lesson data. The sanitizer keeps a bounded HTML and
inline-SVG vocabulary, scopes author CSS to the owning block, and rejects anything
that could execute, fetch, or escape the compiler-owned slide layout.
"""
...
# HTMLParser lowercases names; browsers re-adjust foreign content, but emitting the
# canonical spelling keeps the artifact readable by XML tools too.
_SVG_CASED_NAMES = {
...
def resolve_scope(html: str, scope: str) -> str:
    return html.replace(SCOPE_PLACEHOLDER, scope)
...
class _SlideHtmlSanitizer(HTMLParser):
    def __init__(self) -> None:
        ...
        # Each level holds (source tag, emitted tag); None unwraps the tag but keeps
        # its children, which is how unsupported elements degrade instead of failing.

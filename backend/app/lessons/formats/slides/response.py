"""Recover and validate a structured Slides model response."""
...
_QUESTION_FIELDS = ("question", "choices", "answer_index", "explanation")
...
def _normalize_slides_dict(data: dict[str, object]) -> None:
    ...
    for slide in slides:
        ...
        if isinstance(body, dict):
            slide["body"] = [body]
        if slide.get("kind") == "comprehension-check":
            if body in ([], None):
                slide.pop("body", None)
            continue
        for field in _QUESTION_FIELDS:
            value = slide.get(field, None)
            if value in (None, "", []):

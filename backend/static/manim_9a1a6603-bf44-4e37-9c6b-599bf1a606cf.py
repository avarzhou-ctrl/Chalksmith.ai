...
class MainScene(Scene):
    def construct(self):
        ...
        # Hour mappings: x-coordinate = -4.0 + (hour - 8.0) * 2.0
        # 8:00 -> -4.0, 9:00 -> -2.0, 10:00 -> 0.0, 11:00 -> 2.0, 12:00 -> 4.0
        hours = [8, 9, 10, 11, 12]
        for hour in hours:
            x_pos = -4.0 + (hour - 8.0) * 2.0
            tick = Line(
            ...
            label = Text(f"{hour}:00", font_size=16, color=WHITE)
        ...
        # -------------------------------------------------------------
        # Start Time: 8:15 AM -> x = -4.0 + 0.25 * 2.0 = -3.5
        start_x = -3.5
        start_dot = Dot(point=np.array([start_x, timeline_y, 0]), color=YELLOW, radius=0.12)
        start_label = Text("8:15 AM\n(Start)", font_size=14, color=YELLOW).next_to(start_dot, DOWN, buff=0.8)
        
        # End Time: 10:45 AM -> x = 0.0 + 0.75 * 2.0 = 1.5
        end_x = 1.5
        end_dot = Dot(point=np.array([end_x, timeline_y, 0]), color=RED, radius=0.12)
        end_label = Text("10:45 AM\n(End)", font_size=14, color=RED).next_to(end_dot, DOWN, buff=0.8)
        ...
        curve1, lbl1 = self.get_jump_bezier(start_x, -2.0, "+45 min", height=0.6, y_base=timeline_y)
        
        self.play(ReplacementTransform(step_instruction, step1_desc))
        ...
        curve2, lbl2 = self.get_jump_bezier(-2.0, 0.0, "+1 hr", height=0.8, y_base=timeline_y)
        
        self.play(ReplacementTransform(step1_desc, step2_desc))
        ...
        curve3, lbl3 = self.get_jump_bezier(0.0, end_x, "+45 min", height=0.6, y_base=timeline_y)
        
        self.play(ReplacementTransform(step2_desc, step3_desc))
        ...
        self.play(ReplacementTransform(step3_desc, step4_desc))
        self.wait(1)
        ...
        calc3.next_to(calc2, DOWN, buff=0.2)
        ...
        # -------------------------------------------------------------
        # Fade out everything from the workspace
        self.play(
            FadeOut(timeline),
            FadeOut(ticks),
            FadeOut(tick_labels),
        ...
        # Clean up scene termination safely using list comprehension
        self.play(*[FadeOut(mob) for mob in self.mobjects])
        self.wait(1)

    def get_jump_bezier(self, start_x, end_x, label_text, height=0.6, y_base=-1.5):

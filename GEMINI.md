...
- **2026-04-16**: Enhanced the "Auto-Fix" system to capture and display dynamic error messages from both the backend (rendering failures) and frontend (JSON parse errors). The error modal now provides the specific technical reason for failure, which is automatically fed back to the LLM during the repair prompt.